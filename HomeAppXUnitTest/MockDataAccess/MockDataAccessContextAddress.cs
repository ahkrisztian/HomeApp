    ...
    public class MockDataAccessContextAddress : IAddressData
    {
        ...
        public MockDataAccessContextAddress(IMapper mapper)
        {
            users.Add(new UserModel()
            {
                ...
                Email = "jones@tom.com",
                ObjectId = "1",
                Addresses = new List<AddressModel>() { new AddressModel() { Id = 1, Country = "Germany", City = "Berlin", Street = "Tut Str. 3", UserId = 1 } },
                HomeModels = new List<HomeModel>(){ new HomeModel() { Id = 1, AddressId = 1, UserId = 1, Name = "Jones Home", Description = "Jones Home 1",
            ...
            users.Add(new UserModel()
            {
                ...
                ObjectId = "2",
                Addresses = new List<AddressModel>() { new AddressModel() { Id = 2, Country = "Germany", City = "Hamburg", Street = "Burg Str. 77", UserId = 2 } },
                HomeModels = new List<HomeModel>(){ new HomeModel() { Id = 2, AddressId = 2, UserId = 2, Name = "Little Home", Description = "Little Home 1",
        ...
        public Task AddAddressByUserId(ReadAddressDTO addressDTO)
        {
            return Task.Run(() =>
            {
                users.Where(i => i.Id == addressDTO.UserId).FirstOrDefault().Addresses.Add(mapper.Map<AddressModel>(addressDTO));
        ...
        public Task DeleteAddressByUserId(int id)
        {
            return Task.Run(() =>
            {
                var address = users.Where(i => i.Id == id).FirstOrDefault().Addresses.Where(u => u.UserId == id).FirstOrDefault();
        ...
        public Task<List<ReadAddressDTO>> UpdateUser(AddressModel address)
        {
            return Task.Run(() =>
            {
                int index = users.Where(i => i.Id == address.UserId).FirstOrDefault().Addresses.FindIndex(i => i.Id == address.Id);

                users.Where(i => i.Id == address.UserId).FirstOrDefault().Addresses[index] = address;

                var result = users.Where(i => i.Id == address.UserId).FirstOrDefault().Addresses.Where(i => i.Id == address.Id);
